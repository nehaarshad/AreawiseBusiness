...
class setShopAreaDropDown extends ConsumerStatefulWidget {
  final String userid;

  setShopAreaDropDown({required this.userid, Key? key}) : super(key: key);

  @override
  ConsumerState<setShopAreaDropDown> createState() => _setShopAreaDropDown();
}
...
class _setShopAreaDropDown extends ConsumerState<setShopAreaDropDown> {
  ...
  void RecommendedArea() {
    ...
    if (addnewArea) {
      ref.read(addShopProvider(widget.userid).notifier).toggleCustomArea(true);
    } else {
      ref.read(addShopProvider(widget.userid).notifier).toggleCustomArea(false);
    }
  }
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: shopArea.text.isNotEmpty
              ...
              onPressed: () {
                ...
                ref.read(addShopProvider(widget.userid).notifier).setLocation(null);
              },
            ): null,
          ),
          onChanged: (value) {
            ref.read(addShopProvider(widget.userid).notifier).setLocation(null); // Reset category while typing
            RecommendedArea();
          },
          onTap: () {
            setState(() {
...
class updateShopAreaDropDown extends ConsumerStatefulWidget {
  ...
  updateShopAreaDropDown({required this.userid, Key? key}) : super(key: key);
...
class _updateShopAreaDropDown extends ConsumerState<updateShopAreaDropDown> {
  ...
  void didChangeDependencies() {
    ...
    if (state.value?.category != null) {
      shopArea.text = state.value!.sector!;
      shopArea.addListener(RecommendedArea);
    }
  }
  ...
  void RecommendedArea() {
    ...
    if (addnewArea) {
      ref.read(updateShopProvider(widget.userid).notifier).toggleCustomArea(true);
       }
    else {
      ref.read(updateShopProvider(widget.userid).notifier).toggleCustomArea(false);
    }
  }
  ...
  void _onAreaSelected(String area) {
    ...
    ref.read(updateShopProvider(widget.userid).notifier).setArea(area);
    focus.unfocus();
  }
