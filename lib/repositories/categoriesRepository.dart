...
class CategoriesRepositories{
  ...
  Future<List<Category>> getCategories() async{
    ...
    try{
      dynamic response=await apiservice.GetApiResponce(AppApis.getCategoriesEndPoints);
      if (response is List){
        return response.map((category)=>Category.fromJson(category as Map<String,dynamic>)).toList();
      }
  ...
  Future<List<Subcategory>> FindSubCategories(String category) async{
    ...
    try{
      dynamic response=await apiservice.GetApiResponce(AppApis.getSubcategoriesOfCategoryEndPoints.replaceFirst(':categories', category));
      if (response is List){
        return response.map((subcategories)=>Subcategory.fromJson(subcategories as Map<String,dynamic>)).toList();
      }
