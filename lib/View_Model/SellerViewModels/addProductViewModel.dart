...
class AddProductViewModel extends StateNotifier<ProductState> {
  ...
  Future<void> findSubcategories(String category) async {
    try {
      ...
      final subcategories = await ref.read(categoryProvider).FindSubCategories(category);
  ...
  Future<void> pickImages(BuildContext context) async {
    try {
      ...
      if (pickedFiles.isNotEmpty && state.images.length + pickedFiles.length <= 7) {
        ...
        final newImages = [...state.images, ...pickedFiles.map((x) => File(x.path))];
  ...
  void removeImage(int index) {
    final newImages = List<File>.from(state.images)..removeAt(index);  //[..] Cascade or Chaining Opertor in list
    state = state.copyWith(images: newImages);
  }

  void setCategory(Category? category) {
    state = state.copyWith( selectedCategory: category, selectedSubcategory: null, subcategories: [] );
    if (category != null) {
  ...
  }) async
  {
    try {
      ...
      final data = {
        ...
        'productcategory': state.selectedCategory?.name ?? state.customCategoryName,
        'productsubcategory': state.selectedSubcategory?.name ?? state.customSubcategoryName,
      };
      ...
      final response = await ref.read(productProvider).addProduct(data, shopId.toString(),state.images.whereType<File>().toList());
      final addproduct = ProductModel.fromJson(response);
      state=state.copyWith(product: AsyncValue.data(addproduct),isLoading: false);
      Utils.flushBarErrorMessage("Product Created!", context);
       Navigator.pop(context);
    }
  catch(e){
    Utils.flushBarErrorMessage('${e}', context);
    state = state.copyWith( product: AsyncValue.error(e, StackTrace.current), isLoading: false);
  }
  }
}
