...
class AddShopViewModel extends StateNotifier<ShopState> {
  ...
  Future<void> pickImages(BuildContext context) async {
    try {
      ...
      if (pickedFiles.isNotEmpty && state.images.length + pickedFiles.length > 4) {
  ...
  void toggleCustomCategory(bool value) {
    state = state.copyWith(
      ...
      customCategoryName: value ? null : state.customCategoryName, //null to initialize its value in other function
    );
  }
  ...
  }) async
  {
    try {
      ...
      state = state.copyWith( shop: AsyncValue.error(e, StackTrace.current), isLoading: false);
    }
  }
}
