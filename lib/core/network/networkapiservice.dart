...
class networkapiservice extends baseapiservice{
  ...
  @override
  Future PostApiWithJson(String url, dynamic data,  Map<String, String> headers) async{
    dynamic responseJson;
    try{
      Response response= await post(Uri.parse(url), body: data , headers: headers ).timeout(Duration(seconds: 30));
  ...
  Future UpdateApiWithJson(String url, dynamic data,  Map<String, String> headers) async{
    dynamic responseJson;
    try{
      Response response= await put(Uri.parse(url), body: data , headers: headers ).timeout(Duration(seconds: 30));
  ...
  Future PostApiWithMultiport(String url, Map<String, dynamic> data, List<File>? files ) async{

    try{
      final request = new http.MultipartRequest('POST', Uri.parse(url));

      data.forEach((key, value) {  //to add data into fields
      ...
      final streameresponse = await request.send().timeout(Duration(seconds: 10));
  ...
  Future UpdateApiWithMultiport(String url, Map<String, dynamic> data, List<File>? files ) async{
  ...
  Future<Map<String, dynamic>> SingleFileUpdateApiWithMultiport(String url, Map<String, dynamic> data, File? files ) async{
  ...
  Future DeleteApiResponce(String url ) async{
    dynamic responseJson;
    try{
      Response response= await delete(Uri.parse(url) ).timeout(Duration(seconds: 10));
  ...
  dynamic httpResponse(http.Response res){
    switch (res.statusCode){
      ...
      default:
        throw fetchdataException("Error occur while communication with server \t Status Code:"+res.statusCode.toString());
    }
  }
